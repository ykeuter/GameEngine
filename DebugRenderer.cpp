...
void GameEngine::DebugRenderer::end() {
    glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
    // Orphan the buffer
...
void GameEngine::DebugRenderer::drawBox(const glm::vec4& destRect, const ColorRGBA8& color, float angle) {
...
void GameEngine::DebugRenderer::drawCircle(const glm::vec2& center, const ColorRGBA8& color, float radius) {
...
void GameEngine::DebugRenderer::render(const glm::mat4& projectionMatrix, float lineWidth) {
    m_program.use();
...
void GameEngine::DebugRenderer::dispose() {
    if (m_vao) {
